...
i = as.numeric(args[1])
bxy = as.numeric(args[2])
f = as.numeric(args[3])
gamma_x = as.numeric(args[4])
gamma_u = as.numeric(args[5])
rstfile = args[6]
...
# x1 - eQTL1 (cause SCZ), y1 - SCZ
# x2 - eQTL2 (cause ACE), y2 - ACE

set.seed(i + f*100);
n1 = 32000; n2 = 500000

# f = runif(1, 0.01, 0.5)
hzx = 0.0025; bzx = sqrt(hzx/(2*f*(1-f)))
bux = 1; hux = 0.3;
hxy = bxy^2
buy = -1; huy = 0.3;
...
x1 = z1*bzx + scale(bux*u)[,1]*sqrt(hux) + scale(rnorm(n1, 0, 1))[,1]*sqrt(1-hzx-hux);
x1 = scale(x1)[,1]
# dataset 2, GWAS, z -> ACE -> SCZ
z2 = rbinom(n2, 2, f);
u = scale(rnorm(n2, 0, 1))[,1]
x2 = z2*bzx + scale(bux*u)[,1]*sqrt(hux) + scale(rnorm(n2, 0, 1))[,1]*sqrt(1-hzx-hux);
x2 = scale(x2)[,1];
cov_xy = sqrt(hux)*sqrt(hxy)*sign(bux)*sign(bxy) * sqrt(huy)*sign(buy)
y2 = x2*bxy + scale(buy*u)[,1]*sqrt(huy) + scale(rnorm(n2, 0, 1))[,1]*sqrt(1-hxy-huy-2*cov_xy);
y2 = scale(y2)[,1]
indexbuf = which((y2 >= qnorm(p, lower.tail=F)))
y2[indexbuf] = 1; y2[-indexbuf] = 0
...
s_prob = gamma_0 + gamma_u*u + gamma_x*x2
s_prob = exp(s_prob)/(1+exp(s_prob))
s = rbinom(n2, 1, s_prob)
